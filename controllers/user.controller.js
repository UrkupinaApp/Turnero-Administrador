...
const userRegister = async (req, res) => {
  try {
    const {
      name, apellido, password, celular, dni, email = '',
      tipo_propietario, uso, propietario_asignado, fila, pasillo, puesto, tamano_puesto
      // Si es propietario, además: puestos, inquilinos, etc.
    } = req.body;
    ...
    connection.query(
      ...
      async (err, results) => {
        ...
        if (tipo_propietario === "PROPIETARIO") {
          // ...la lógica actual que recorre puestos e inquilinos...
          // (dejamos como está, o adaptamos si tu front envía array de puestos)
          // ¡Si querés te la escribo adaptada!
        }

        // === 2. SOCIO/INQUILINO ===
        if (tipo_propietario === "SOCIO") {
          if (!propietario_asignado) {
            return res.status(400).json({ message: "Debe seleccionar un propietario del puesto" });
          }
          // Info mínima del puesto, adaptá según lo que pide tu negocio:
          if (!fila || !pasillo || !puesto || !tamano_puesto) {
            return res.status(400).json({ message: "Datos del puesto incompletos" });
          }

          const socioData = [
            name, apellido, hashedPass, 13, celular, dni, email,
            fila, pasillo, puesto, "INQUILINO", uso || "USO PROPIO", tamano_puesto,
            null, propietario_asignado // inquilinos, user_id_propietario
          ];
          connection.query(
            `INSERT INTO users 
              (name, apellido, password, creditos, celular, dni, email, fila, pasillo, puesto, tipo_propietario, uso, tamano_puesto, inquilinos, user_id_propietario)
            ...
            (err2, result) => {
              if (err2) {
                console.error("Error al insertar socio/inquilino:", err2);
                return res.status(500).json({ message: "Error al registrar el usuario" });
              }
              return res.status(201).json({ message: "Socio/Inquilino registrado" });
            }
          );
        }
      }
